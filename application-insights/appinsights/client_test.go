...
func TestClientPropertiesWithConnectionString(t *testing.T) {
	client := NewTelemetryClientFromConfig(NewTelemetryConfigurationWithConnectionString(connection_string))
	defer client.Channel().Close()
	...
	if ikey := client.InstrumentationKey(); ikey != "00000000-0000-0000-0000-000000000000" {
		t.Error("Client's InstrumentationKey is not expected")
	}

	if ikey := client.Context().InstrumentationKey(); ikey != "00000000-0000-0000-0000-000000000000" {
		t.Error("Context's InstrumentationKey is not expected")
	}
	...
	if client.Channel().EndpointAddress() != "https://ingestion.endpoint.com/v2/track" {
		t.Error("Client.Channel.EndpointAddress was incorrect")
	}
}
...
func TestEndToEndWithConnectionString(t *testing.T) {
	mockClock(time.Unix(1511001321, 0))
	defer resetClock()
	xmit, server := newTestClientServer()
	defer server.Close()

	baseEndpoint := strings.TrimSuffix(xmit.(*httpTransmitter).endpoint, "v2/track")
	testConnectionString := fmt.Sprintf("InstrumentationKey=%s;IngestionEndpoint=%s", test_ikey, baseEndpoint)
	client := NewTelemetryClientFromConfig(NewTelemetryConfigurationWithConnectionString(testConnectionString))
	defer client.Channel().Close()
