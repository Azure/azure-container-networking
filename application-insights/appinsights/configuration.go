...
// Creates a new TelemetryConfiguration object with the specified
// connection string and endpoint URL.
func NewTelemetryConfigurationWithConnectionString(connectionString string) *TelemetryConfiguration {
	connectionParams, err := parseConnectionString(connectionString)
	if err != nil {
		return nil
	}

	return &TelemetryConfiguration{
		InstrumentationKey: connectionParams.InstrumentationKey,
		EndpointUrl:        connectionParams.IngestionEndpoint,
		MaxBatchSize:       1024,
		MaxBatchInterval:   time.Duration(10) * time.Second,
	}
}
