...
func TestGenerateMultitenantTransparentVlanConflist(t *testing.T) {
	fixture := "testdata/fixtures/azure-linux-multitenant-transparent-vlan.conflist"

	buffer := new(bytes.Buffer)
	g := cniconflist.MultitenantTransparentVLANGenerator{Writer: &bufferWriteCloser{buffer}}
	err := g.Generate()
	assert.NoError(t, err)
