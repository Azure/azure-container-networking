...
func TestEndpointHandlerAPI_PatchMethod(t *testing.T) {
	...
	req := httptest.NewRequestWithContext(context.Background(), http.MethodPatch, cns.EndpointPath+containerID, bytes.NewReader(reqBody))
...
func TestEndpointHandlerAPI_OptManageEndpointStateDisabled(t *testing.T) {
	...
	req := httptest.NewRequestWithContext(context.Background(), http.MethodGet, cns.EndpointPath+"test-container", http.NoBody)
	w := httptest.NewRecorder()
