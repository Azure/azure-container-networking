...
// normalizeMAC lowercases and converts hyphen-delimited MACs to colon-delimited.
func normalizeMAC(mac string) string {
	return strings.ToLower(strings.ReplaceAll(mac, "-", ":"))
}

// findEndpointStateByMAC returns the first EndpointState entry with a delegated NIC matching the given MAC.
// A MAC corresponds to a single physical NIC, so at most one endpoint can match.
...
func (service *HTTPRestService) cleanupStaleHNSResources(ncID, mac string) (returnErr error) {
	...
	if ipInfo.HnsNetworkID != "" {
		if err := service.hnsClient.DeleteNetworkByID(ipInfo.HnsNetworkID); err != nil {
