...
// TestBuildAndCreateNodeInfo_EnableHomeAz verifies that buildAndCreateNodeInfo
// correctly creates a NodeInfo CRD when EnableHomeAz is set.
func TestBuildAndCreateNodeInfo_EnableHomeAz(t *testing.T) {
	tests := []struct {
		name        string
		vmUniqueID  string
		homeAz      uint
		...
		{
			name:        "EnableHomeAz with valid HomeAZ",
			vmUniqueID:  "homeaz-vm-id",
			homeAz:      3,
			expectedAZ:  "AZ03",
			expectError: false,
		},
		{
			name:        "EnableHomeAz with no HomeAZ",
			vmUniqueID:  "homeaz-vm-no-az",
			homeAz:      0,
			expectedAZ:  "",
			expectError: false,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, "homeaz-test-node", nodeInfoCli.createdNodeInfo.Name)
		})
	}
}
