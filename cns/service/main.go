...
func reconcileInitialCNSState(nnc *v1alpha.NodeNetworkConfig, ipamReconciler ipamStateReconciler, podInfoByIPProvider cns.PodInfoByIPProvider) error {
...
func InitializeCRDState(ctx context.Context, z *zap.Logger, httpRestService cns.HTTPService, cnsconfig *configuration.CNSConfig) error {
	...
	initializerWrapper := func(nnc *v1alpha.NodeNetworkConfig) error {
		z.Info("reconciling initial CNS state")
		if initErr := reconcileInitialCNSState(nnc, httpRestServiceImplementation, podInfoByIPProvider); initErr != nil {
			return initErr
		}
		return nil
	}
	...
	nncReconciler := nncctrl.NewReconciler(httpRestServiceImplementation, initializerWrapper, poolMonitor, nodeIP)
