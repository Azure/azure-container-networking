...
func (rc *RequestControllerFake) Reconcile(removePendingReleaseIPs bool) error {
	diff := int(rc.NNC.Spec.RequestedIPCount) - len(rc.fakecns.GetPodIPConfigState())
	...
	if removePendingReleaseIPs {
		rc.fakecns.IPStateManager.RemovePendingReleaseIPConfigs(rc.NNC.Spec.IPsNotInUse)
	}

	// update
	rc.fakecns.PoolMonitor.Update(*rc.NNC)
