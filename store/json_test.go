...
func TestKeyValuePairsAreReinstantiatedFromJSONFile(t *testing.T) {
	...
	kvs, err := NewJsonFileStore(testFileName, processlock.NewMockFileLock(false), false)
	if err != nil {
...
func TestNewJsonFileStoreIdempotent(t *testing.T) {
	_, err := NewJsonFileStore(testLockFileName, processlock.NewMockFileLock(false), false)
	if err != nil {
	...
	tests := []struct {
	...
	}{
		{
			...
			store: func() KeyValueStore {
				st, _ := NewJsonFileStore(testFileName, processlock.NewMockFileLock(false), false)
				return st
			}(),
			timeoutms: 10000,
			wantErr:   false,
		},
		{
			...
			store: func() KeyValueStore {
				st, _ := NewJsonFileStore(testFileName, processlock.NewMockFileLock(true), false)
				return st
			}(),
			timeoutms:  10000,
			wantErr:    true,
...
func TestFileName(t *testing.T) {
	_, err := NewJsonFileStore("", processlock.NewMockFileLock(false), false)
	if err == nil {
		t.Errorf("This should have failed for empty file name")
	}

	_, err = NewJsonFileStore("test.json", processlock.NewMockFileLock(false), false)
	if err != nil {
