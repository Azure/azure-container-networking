...
// TestRunGoldenRegressionInfraNotRegression is the golden eval: a datapath-stage
// failure whose top-line signal is CNS AllocateIPConfig / connection refused, but
// the collected pod dumps show a kube-system Defender DaemonSet init container in
// CrashLoopBackOff (init exit 1 from an embedded install script), identical across
// two nodepools and two networking stages on the same commit and image tag. The
// correct verdict is an environmental/infra failure owned by the node-image team,
// not a CNS/CNI regression, and CNS "connection refused" is a symptom. This test
// exercises the full pipeline over the committed golden bundle and asserts the
// contract is preserved end to end.
func TestRunGoldenRegressionInfraNotRegression(t *testing.T) {
	out := t.TempDir()
	cl := &fakeClassifier{result: goldenInfraClassification()}
	opts := options{
		input:          filepath.Join("testdata", "golden-defender-crashloop"),
		output:         out,
		signaturesPath: filepath.Join("signatures", "signatures.yaml"),
		dryRun:         true,
		pipeline:       "ACN PR",
	...
	inc := readIncident(t, out)
	if inc.Category == model.CategoryPRRegression {
		t.Fatal("golden case must not be classified pr_regression")
	}
	if inc.Category != model.CategoryPipelineInfraConfig {
		t.Errorf("category: got %s, want pipeline_infra_config", inc.Category)
	}
	if !strings.Contains(inc.FailingUnit, "install-packages.sh") {
		t.Errorf("failingUnit should name the init installer, got %q", inc.FailingUnit)
	}

	// The deterministic pre-match must surface the infra init-crashloop signature
	// even though CNS pr_regression signatures also match (the trap).
	var sawInfraSig bool
	for _, m := range inc.SignatureMatches {
		if m.ID == "daemonset-init-container-crashloop-package-install" {
			sawInfraSig = true
		}
	}
	if !sawInfraSig {
		t.Errorf("expected the init-crashloop infra signature among matches, got %v", sigIDs(inc.SignatureMatches))
	}
	...
	for _, want := range []string{"### Symptom vs cause", "connection refused", "### Falsification", "refuted", "### Evidence gaps", "--previous"} {
		if !strings.Contains(string(md), want) {
			t.Errorf("expected %q in golden report.md", want)
		}
	}
}

func sigIDs(matches []model.SignatureMatch) []string {
...
func goldenInfraClassification() model.Classification {
	return model.Classification{
		Category:         model.CategoryPipelineInfraConfig,
		Confidence:       0.72,
		RootCauseSummary: "The AzSecPack/Defender init container's install-packages.sh exited 1 under chroot, crashlooping the DaemonSet and blocking kube-system readiness.",
		TopAnomaly:       "kube-system Defender DaemonSet init container CrashLoopBackOff (x289 over 75m), init exit code 1",
		FailingUnit:      "install-packages.sh in the azsecpack init-package-installer container (azsecpack-azl3-image:1.0.034761)",
		RecommendedOwner: "aks-node-image",
		TopEvidence:      []string{"install-packages.sh failed with exit code", "Init:CrashLoopBackOff x289"},
		CausalChain: []model.CausalHop{
			{Step: "Both nodes rebooted", Timestamp: "2026-07-27T08:59:00Z", Citation: "node-conditions.txt Kubelet start"},
			{Step: "install-packages.sh exited 1 under chroot", Citation: "bad-pod-describe.txt line 1048"},
			{Step: "init container CrashLoopBackOff blocked SynchronizedBeforeSuite", Citation: "pods.txt"},
		},
		SymptomVsCause: []model.SymptomCause{
			{Signal: "CNS AllocateIPConfig / connection refused", Classification: "symptom", Justification: "CNS restarted during the same node disruption; API briefly unreachable"},
			{Signal: "install-packages.sh exit 1", Classification: "cause", Justification: "primary init failure quoted from the embedded script exit path"},
		},
		Falsification: &model.Falsification{
			Hypothesis:        "CNS IPAM code regression (pr_regression)",
			IfTrueExpect:      "failure localized to CNS on the changed datapath, varying by stage/commit",
		...
		EvidenceGaps: []model.EvidenceGap{
			{Missing: "literal dpkg PackageInstallFailed message", WhereItLives: "expired k8s event and /upper/upper/tmp/install-packages.log", WhyMissing: "events ~1h TTL; capture was 75m in", HowToCapture: "kubectl logs <pod> -c init-package-installer --previous"},
		},
		KnownUnknowns: []string{"exact failing .deb postinst step not captured this run"},
		ProposedFix:   "Route to node-image/AzSecPack; on repro capture the installer log before the event TTL.",
		Source:        "llm",
	}
}
