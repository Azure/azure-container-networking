...
func (client *OVSEndpointClient) AddEndpointRules(epInfo *EndpointInfo) error {
	...
	if err := ovsClient.AddPortOnOVSBridge(client.hostVethName, client.bridgeName, client.vlanID); err != nil {
		return err
	}

	log.Printf("[ovs] Get ovs port for interface %v.", client.hostVethName)
	containerOVSPort, err := ovsClinet.GetOVSPortNumber(client.hostVethName)
	if err != nil {
	...
	hostPort, err := ovsClient.GetOVSPortNumber(client.hostPrimaryIfName)
	if err != nil {
		log.Printf("[ovs] Get ofport failed with error %v", err)
		return err
	}
	...
	for _, ipAddr := range epInfo.IPAddresses {
		...
		if err := ovsClient.AddFakeArpReply(client.bridgeName, ipAddr.IP); err != nil {
			return err
		}
		...
		if err := ovsClient.AddIpSnatRule(client.bridgeName, ipAddr.IP, client.vlanID, containerOVSPort, client.hostPrimaryMac, hostPort); err != nil {
			return err
		}
		...
		if err := ovsClient.AddMacDnatRule(client.bridgeName, hostPort, ipAddr.IP, client.containerMac, client.vlanID, containerOVSPort); err != nil {
			return err
		}
	}
...
func (client *OVSEndpointClient) DeleteEndpointRules(ep *endpoint) {
	...
	containerPort, err := ovsClient.GetOVSPortNumber(client.hostVethName)
	if err != nil {
	...
	ovsClient.DeleteIPSnatRule(client.bridgeName, containerPort)
	...
	ovsClient.DeleteArpReplyRule(client.bridgeName, containerPort, ep.IPAddresses[0].IP, ep.VlanID)
	...
	ovsClient.DeleteMacDnatRule(client.bridgeName, hostPort, ep.IPAddresses[0].IP, ep.VlanID)

	// Delete port from ovs bridge
	log.Printf("[ovs] Deleting interface %v from bridge %v", client.hostVethName, client.bridgeName)
	ovsClient.DeletePortFromOVS(client.bridgeName, client.hostVethName)
