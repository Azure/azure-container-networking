...
func TestConfigureHCNNetworkRouteByFamily(t *testing.T) {
	tests := []struct {
		name                 string
		subnets              []SubnetInfo
	...
	}{
		{
			...
			subnets: []SubnetInfo{
				{
					Family:  platform.AfINET,
					Prefix:  net.IPNet{IP: net.ParseIP("10.240.0.0"), Mask: net.CIDRMask(12, 32)},
					Gateway: net.ParseIP("10.240.0.1"),
				},
			},
			expectedDestinations: []string{defaultRouteCIDR},
		},
		{
			...
			subnets: []SubnetInfo{
				{
					Family:  platform.AfINET6,
					Prefix:  net.IPNet{IP: net.ParseIP("fc00::"), Mask: net.CIDRMask(64, 128)},
					Gateway: net.ParseIP("fc00::1"),
				},
			},
			expectedDestinations: []string{defaultIPv6Route},
		},
		{
			name: "dual stack subnet list uses per family default route",
			subnets: []SubnetInfo{
				{
					Family:  platform.AfINET,
					Prefix:  net.IPNet{IP: net.ParseIP("10.240.0.0"), Mask: net.CIDRMask(12, 32)},
			...
			expectedDestinations: []string{defaultRouteCIDR, defaultIPv6Route},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if len(hostComputeNetwork.Ipams) != 1 {
				t.Fatalf("expected exactly 1 ipam, got %d", len(hostComputeNetwork.Ipams))
			}

			gotSubnets := hostComputeNetwork.Ipams[0].Subnets
			if len(gotSubnets) != len(tt.subnets) {
				t.Fatalf("expected %d subnets, got %d", len(tt.subnets), len(gotSubnets))
			}

			for i := range gotSubnets {
				if len(gotSubnets[i].Routes) != 1 {
					t.Fatalf("expected 1 route in subnet %d, got %d", i, len(gotSubnets[i].Routes))
				}

				if gotSubnets[i].Routes[0].DestinationPrefix != tt.expectedDestinations[i] {
					t.Fatalf("unexpected destination prefix for subnet %d: got %s, want %s", i, gotSubnets[i].Routes[0].DestinationPrefix, tt.expectedDestinations[i])
				}
			}
		})
	}
