...
// fieldTagMapper is a translation function from a slog attribute name to an Application Insights tag name.
// fieldTagMappers can be used to transform conventional names like "version" from a slog attribute name
// into the magic Application Insights field key "ai.application.ver".
...
// DefaultMappers combines all the default context mappers into a single map.
var DefaultMappers = func() map[string]string {
	result := make(map[string]string)

	maps.Copy(result, ApplicationContextMappers)
	maps.Copy(result, DeviceContextMappers)
	maps.Copy(result, LocationContextMappers)
	maps.Copy(result, OperationContextMappers)
	maps.Copy(result, SessionContextMappers)
	maps.Copy(result, UserContextMappers)
	maps.Copy(result, CloudContextMappers)
	maps.Copy(result, InternalContextMappers)
