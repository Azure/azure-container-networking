...
# run all tests
# COVER_PKG omits folders with all files tagged with one of 'unit', '!ignore_test', '!ignore_cli', '!ignore_crd', or '!ignore_autogenerated'
.PHONY: test-all
test-all:
	@$(eval COVER_PKG=`go list --tags ignore_test,ignore_cli,ignore_crd,ignore_autogenerated ./... | tr '\n' ','`)
	@echo Test coverpkg: $(COVER_PKG)
	go test -tags "unit" -coverpkg=$(COVER_PKG) -v -race -covermode atomic -failfast -coverprofile=coverage.out ./...
