...
$(CNS_BUILD_DIR)/azure-cns$(EXE_EXT): $(CNSFILES)
	CGO_ENABLED=0 go build --tags crd -v -o $(CNS_BUILD_DIR)/azure-cns$(EXE_EXT) -ldflags "-X main.version=$(VERSION) -X $(cnsaipath)=$(CNS_AI_ID)" -gcflags="-dwarflocationlists=true" $(CNS_DIR)/*.go
...
# run all tests
# COVER_PKG omits files tagged with 'test', 'cli', '!ignore_crd', '!ignore_autogenerated', or 'unit'
.PHONY: test-all
test-all:
	@$(eval COVER_PKG=`go list --tags ignore_crd,ignore_autogenerated ./... | tr '\n' ','`)
	@echo Test coverpkg: $(COVER_PKG)
	go test -tags "test,cli,crd,unit" -coverpkg=$(COVER_PKG) -v -race -covermode atomic -failfast -coverprofile=coverage.out ./...
