...
CNI_WINDOWS_PLATFORM_TAG		?= $(subst /,-,$(PLATFORM))$(if $(OS_VERSION),-$(OS_VERSION),)-$(CNI_VERSION)-$(WINDOWS_OS_SKU)
...
npm-image: ## build the npm container image.
	$(MAKE) container-$(CONTAINER_BUILDER) \
		...
		IMAGE=$(NPM_IMAGE) \
		EXTRA_BUILD_ARGS='--build-arg NPM_AI_PATH=$(NPM_AI_PATH) --build-arg ARCH=$(ARCH) --build-arg NPM_AI_ID=$(NPM_AI_ID) --build-arg OS_VERSION=$(OS_VERSION)' \
		PLATFORM=$(PLATFORM) \
