...
func TestCreateEpInfoFallsBackToInterfacePrimaryIPForSingleTenant(t *testing.T) {
	resources := GetTestResources()
	plugin := &NetPlugin{
		Plugin: resources.Plugin,
		nm:     network.NewMockNetworkmanager(network.NewMockEndpointClient(nil)),
	...
	opt := &createEpInfoOpt{
		...
		ipamAddResult:    IPAMAddResult{interfaceInfo: map[string]network.InterfaceInfo{}},
		azIpamResult:     nil,
		args: &cniSkel.CmdArgs{
			ContainerID: "test-container",
			Netns:       "bc526fae-4ba0-4e80-bc90-ad721e5850bf",
		...
		ipamAddConfig: &IPAMAddConfig{
			nwCfg:   nwCfg,
			args:    &cniSkel.CmdArgs{Netns: "bc526fae-4ba0-4e80-bc90-ad721e5850bf"},
	...
	require.NoError(t, err)
	require.Equal(t, ifInfo.PrimaryInterfaceIP, epInfo.PrimaryInterfaceIP)
}
