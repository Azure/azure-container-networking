...
func (plugin *NetPlugin) Start(config *common.PluginConfig) error {
	...
	if err != nil {
		networkLogger.Error("[cni-net] Failed to initialize base plugin", zap.Error(err))
		return err
	}

	// Log platform information.
	networkLogger.Info("Plugin Info",
	...
	if err != nil {
		networkLogger.Error("Failed to initialize network manager", zap.Error(err))
		return err
	}

	networkLogger.Info("Plugin started")
...
func (plugin *NetPlugin) GetAllEndpointState(networkid string) (*api.AzureCNIState, error) {
	...
	if err == store.ErrStoreEmpty {
		networkLogger.Error("failed to retrieve endpoint state", zap.Error(err))
	} else if err != nil {
		return nil, err
	}
...
func (plugin *NetPlugin) Stop() {
	...
	plugin.Uninitialize()
	networkLogger.Info("Plugin stopped")
}
...
func (plugin *NetPlugin) getPodInfo(args string) (name, ns string, err error) {
	...
	if err != nil {
		networkLogger.Error("Error while parsing CNI Args", zap.Error(err))
		return "", "", err
	}
...
func SetCustomDimensions(cniMetric *telemetry.AIMetric, nwCfg *cni.NetworkConfig, err error) {
	if cniMetric == nil {
		networkLogger.Error("Unable to set custom dimension. Report is nil")
		return
	}
...
func addNatIPV6SubnetInfo(nwCfg *cni.NetworkConfig,
	...
	if nwCfg.IPV6Mode == network.IPV6Nat {
		...
		networkLogger.Info("ipv6 subnet info",
...
func (plugin *NetPlugin) Add(args *cniSkel.CmdArgs) error {
	...
	networkLogger.Info("[cni-net] Processing ADD command",
	...
	defer func() {
		...
		if vererr != nil {
			networkLogger.Error("GetAsVersion failed", zap.Error(vererr))
			plugin.Error(vererr)
		}
		...
		networkLogger.Info("[cni-net] ADD command completed for pod %v with IPs:%+v err:%v.",
	...
	if nwCfg.ExecutionMode == string(util.Baremetal) {
		...
		networkLogger.Info("Baremetal mode. Calling vnet agent for ADD")
	...
	for _, ns := range nwCfg.PodNamespaceForDualNetwork {
		if k8sNamespace == ns {
			networkLogger.Info("Enable infravnet for pod",
				zap.String("pod", k8sPodName),
	...
	if nwCfg.MultiTenancy {
		...
		if err != nil {
			...
			networkLogger.Error("GetAllNetworkContainers failed",
				zap.String("pod", k8sPodName),
				zap.String("namespace", k8sNamespace),
				zap.Error(err))
			return err
		...
		if len(ipamAddResults) > 1 && !plugin.isDualNicFeatureSupported(args.Netns) {
			...
			networkLogger.Error("received multiple NC results from CNS while dualnic feature is not supported",
	...
	for i := 0; i < len(ipamAddResults); i++ {
		...
		if nwInfoErr == nil {
			networkLogger.Info("[cni-net] Found network with subnet",
			...
			if err != nil {
				networkLogger.Error("handleConsecutiveAdd failed", zap.Error(err))
				return err
			}
		...
		if nwInfoErr != nil {
			...
			networkLogger.Info("[cni-net] Creating network", zap.String("networkID", networkID))
			...
			if nwInfo, err = plugin.createNetworkInternal(networkID, policies, ipamAddConfig, ipamAddResult); err != nil {
				networkLogger.Error("Create network failed", zap.Error(err))
				return err
			}
			networkLogger.Info("[cni-net] Created network",
		...
		if err != nil {
			networkLogger.Error("Endpoint creation failed", zap.Error(err))
			return err
		}
...
func (plugin *NetPlugin) cleanupAllocationOnError(
	...
	if result != nil && len(result.IPs) > 0 {
		if er := plugin.ipamInvoker.Delete(&result.IPs[0].Address, nwCfg, args, options); er != nil {
			networkLogger.Error("Failed to cleanup ip allocation on failure", zap.Error(er))
		}
	}
	if resultV6 != nil && len(resultV6.IPs) > 0 {
		if er := plugin.ipamInvoker.Delete(&resultV6.IPs[0].Address, nwCfg, args, options); er != nil {
			networkLogger.Error("Failed to cleanup ipv6 allocation on failure", zap.Error(er))
		}
	}
}
...
func (plugin *NetPlugin) createNetworkInternal(
	...
	networkLogger.Info("[cni-net] Found master interface", zap.String("ifname", masterIfName))
	...
	networkLogger.Info("[cni-net] DNS Info", zap.Any("info", nwDNSInfo))
	...
	if err = addSubnetToNetworkInfo(ipamAddResult, &nwInfo); err != nil {
		networkLogger.Info("[cni-net] Failed to add subnets to networkInfo",
			zap.Error(err))
		return nwInfo, err
	}
...
func (plugin *NetPlugin) createEndpointInternal(opt *createEndpointInternalOpt) (network.EndpointInfo, error) {
	...
	if err != nil {
		networkLogger.Error("Failed to get endpoint policies", zap.Error(err))
		return epInfo, err
	}
	...
	if err != nil {
		networkLogger.Error("failed to initialized cns client", zap.String("url", opt.nwCfg.CNSUrl),
			zap.String("error", err.Error()))
		return epInfo, plugin.Errorf(err.Error())
	}

	// Create the endpoint.
	networkLogger.Info("[cni-net] Creating endpoint", zap.String("endpointInfo", epInfo.PrettyString()))
...
func (plugin *NetPlugin) Get(args *cniSkel.CmdArgs) error {
	...
	networkLogger.Info("[cni-net] Processing GET command",
	...
	defer func() {
		...
		networkLogger.Info("[cni-net] GET command completed", zap.Any("result", result),
			zap.Error(err))
	}()
	...
	networkLogger.Info("[cni-net] Read network configuration", zap.Any("config", nwCfg))
	...
	if networkID, err = plugin.getNetworkName(args.Netns, nil, nwCfg); err != nil {
		...
		networkLogger.Error("[cni-net] Failed to extract network name from network config",
			zap.Error(err))
	}
...
func (plugin *NetPlugin) Delete(args *cniSkel.CmdArgs) error {
	...
	networkLogger.Info("[cni-net] Processing DEL command",
		zap.String("containerId", args.ContainerID),
		zap.String("netNS", args.Netns),
		zap.String("ifName", args.IfName),
		zap.Any("args", args.Args),
	...
	defer func() {
		networkLogger.Info("[cni-net] DEL command completed",
			zap.String("pod", k8sPodName),
			zap.Error(err))
	}()
	...
	if k8sPodName, k8sNamespace, err = plugin.getPodInfo(args.Args); err != nil {
		networkLogger.Error("[cni-net] Failed to get POD info", zap.Error(err))
	}
	...
	networkLogger.Info("Execution mode", zap.String("mode", nwCfg.ExecutionMode))
	if nwCfg.ExecutionMode == string(util.Baremetal) {

		networkLogger.Info("Baremetal mode. Calling vnet agent for delete container")
	...
	if plugin.ipamInvoker == nil {
		...
		case network.AzureCNS:
			...
			if cnsErr != nil {
				networkLogger.Error("[cni-net] failed to create cns client", zap.Error(cnsErr))
	...
	networkLogger.Info("[cni-net] Endpoints to be deleted", zap.Int("count", numEndpointsToDelete))
	for i := 0; i < numEndpointsToDelete; i++ {
		...
		if err != nil {
			...
			networkLogger.Error("[cni-net] Failed to extract network name from network config", zap.Error(err))
		...
		if nwInfo, err = plugin.nm.GetNetworkInfo(networkID); err != nil {
			if !nwCfg.MultiTenancy {
				networkLogger.Error("[cni-net] Failed to query network",
					zap.String("network", networkID),
					zap.Error(err))
		...
		// Query the endpoint.
		if epInfo, err = plugin.nm.GetEndpointInfo(networkID, endpointID); err != nil {
			networkLogger.Info("[cni-net] GetEndpoint",
			...
			if !nwCfg.MultiTenancy {
				...
				networkLogger.Error("[cni-net] Failed to query endpoint",
					zap.String("endpoint", endpointID),
					zap.Error(err))
				networkLogger.Error("Release ip by ContainerID (endpoint not found)",
					zap.String("containerID", args.ContainerID))
		...
		networkLogger.Info("Deleting endpoint",
		...
		if !nwCfg.MultiTenancy {
			...
			for i := range epInfo.IPAddresses {
				networkLogger.Info("Release ip", zap.String("ip", epInfo.IPAddresses[i].IP.String()))
...
func (plugin *NetPlugin) Update(args *cniSkel.CmdArgs) error {
	...
	networkLogger.Info("[cni-net] Processing UPDATE command",
		zap.String("netns", args.Netns),
		zap.String("args", args.Args),
		zap.String("path", args.Path))
	...
	defer func() {
		...
		networkLogger.Info("[cni-net] UPDATE command completed",
	...
	if podCfg, err = cni.ParseCniArgs(args.Args); err != nil {
		networkLogger.Error("[cni-net] Error while parsing CNI Args during UPDATE",
			zap.Error(err))
		return err
	}
	...
	networkLogger.Info("Retrieved existing endpoint from state that may get update",
	...
	networkLogger.Info("Going to collect target routes from CNS",
		zap.String("pod", k8sPodName),
		zap.String("namespace", k8sNamespace))
	...
	if orchestratorContext, err = json.Marshal(podInfo); err != nil {
		networkLogger.Error("Marshalling KubernetesPodInfo failed",
			zap.Error(err))
	...
	cnsclient, err := cnscli.New(nwCfg.CNSUrl, defaultRequestTimeout)
	if err != nil {
		networkLogger.Error("failed to initialized cns client",
	...
	if targetNetworkConfig, err = cnsclient.GetNetworkContainer(context.TODO(), orchestratorContext); err != nil {
		networkLogger.Info("GetNetworkContainer failed",
			zap.Error(err))
		return plugin.Errorf(err.Error())
	}

	networkLogger.Info("Network config received from cns",
		zap.String("pod", k8sPodName),
		zap.String("namespace", k8sNamespace),
	...
	networkLogger.Info("Going to collect target routes for from targetNetworkConfig",
		zap.String("pod", k8sPodName),
		zap.String("namespace", k8sNamespace))
	if targetNetworkConfig.Routes != nil && len(targetNetworkConfig.Routes) > 0 {
		for _, route := range targetNetworkConfig.Routes {
			networkLogger.Info("Adding route from routes to targetEpInfo", zap.Any("route", route))
			...
			networkLogger.Info("Successfully added route from routes to targetEpInfo", zap.Any("route", route))
		}
	}

	networkLogger.Info("Going to collect target routes based on Cnetaddressspace from targetNetworkConfig",
		zap.String("pod", k8sPodName),
		zap.String("namespace", k8sNamespace))

	ipconfig := targetNetworkConfig.IPConfiguration
	for _, ipRouteSubnet := range targetNetworkConfig.CnetAddressSpace {
		networkLogger.Info("Adding route from cnetAddressspace to targetEpInfo", zap.Any("subnet", ipRouteSubnet))
		...
		networkLogger.Info("Successfully added route from cnetAddressspace to targetEpInfo", zap.Any("subnet", ipRouteSubnet))
	}

	networkLogger.Info("Finished collecting new routes in targetEpInfo", zap.Any("route", targetEpInfo.Routes))
	networkLogger.Info("Now saving existing infravnetaddress space if needed.")
	for _, ns := range nwCfg.PodNamespaceForDualNetwork {
		if k8sNamespace == ns {
			targetEpInfo.EnableInfraVnet = true
			targetEpInfo.InfraVnetAddressSpace = nwCfg.InfraVnetAddressSpace
			networkLogger.Info("Saving infravnet address space",
	...
	// Update the endpoint.
	networkLogger.Info("Now updating existing endpoint with targetNetworkConfig",
	...
	if netRes.Interfaces != nil {
		for i, ni := range netRes.Interfaces {
			...
			for _, ip := range ni.Ipaddresses {
				...
				if err != nil {
					networkLogger.Error("Error parsing prefix length while converting to cni result",
