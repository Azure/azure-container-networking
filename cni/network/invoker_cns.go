...
func (invoker *CNSIPAMInvoker) Add(addConfig IPAMAddConfig) (IPAMAddResult, error) {
	...
	for i := 0; i < len(response.PodIPInfo); i++ {
		...
		case cns.DelegatedVMNIC, cns.NodeNetworkInterfaceAccelnetFrontendNIC:
			// only handling single v4 PodIPInfo for DelegatedVMNIC and AccelnetNIC at the moment, will have to update once v6 gets added
...
func (invoker *CNSIPAMInvoker) getInterfaceInfoKey(nicType cns.NICType, macAddress string) string {
	if nicType == cns.DelegatedVMNIC || nicType == cns.BackendNIC || nicType == cns.NodeNetworkInterfaceAccelnetFrontendNIC {
		return macAddress
	}
