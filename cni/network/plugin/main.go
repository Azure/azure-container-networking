...
func reportPluginError(reportManager *telemetry.ReportManager, tb *telemetry.TelemetryBuffer, err error) {
	...
	if err := reportManager.SendReport(tb); err != nil {
		log.Logger.Error("SendReport failed", zap.Any("error", err))
	}
}
...
func handleIfCniUpdate(update func(*skel.CmdArgs) error) (bool, error) {
	...
	if err != nil {
		log.Logger.Error("Received error while retrieving cmds from environment", zap.Any("error", err))
		return isupdate, err
	}
	...
	if err != nil {
		log.Logger.Error("Failed to handle CNI UPDATE", zap.Any("error", err))
		return isupdate, err
	}
...
func rootExecute() error {
	...
	if cniCmd != cni.CmdVersion {
		...
		if err = netPlugin.Plugin.InitializeKeyValueStore(&config); err != nil {
			...
			if tberr := tb.Connect(); tberr != nil {
				log.Logger.Error("Cannot connect to telemetry service", zap.Any("error", tberr))
			...
			if errors.Is(err, store.ErrTimeoutLockingStore) {
				...
				if sendErr != nil {
					log.Logger.Error("Couldn't send cnilocktimeout metric", zap.Any("error", sendErr))
				}
			}
		...
		defer func() {
			if errUninit := netPlugin.Plugin.UninitializeKeyValueStore(); errUninit != nil {
				log.Logger.Error("Failed to uninitialize key-value store of network plugin", zap.Any("error", errUninit))
			}
		...
		if cniCmd == cni.CmdGetEndpointsState {
			...
			if err != nil {
				log.Logger.Error("Failed to get Azure CNI state", zap.Any("error", err))
				return errors.Wrap(err, "Get all endpoints error")
			}

			err = simpleState.PrintResult()
			if err != nil {
				log.Logger.Error("Failed to print state result to stdout", zap.Any("error", err))
			}
	...
	if handled {
		...
		log.Logger.Error("Failed to execute network plugin", zap.Any("error", err))
	}
