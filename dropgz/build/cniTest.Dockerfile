...
RUN CGO_ENABLED=0 go build -a -o bin/azure-vnet-telemetry -trimpath -ldflags "-X main.version="$VERSION"" -gcflags="-dwarflocationlists=true" cni/network/plugin/main.go
RUN CGO_ENABLED=0 go build -a -o bin/azure-vnet-ipam -trimpath -ldflags "-X main.version="$VERSION"" -gcflags="-dwarflocationlists=true" cni/network/plugin/main.go
...
COPY --from=azure-vnet /azure-container-networking/bin/azure-vnet pkg/embed/fs
COPY --from=azure-vnet /azure-container-networking/bin/azure-vnet-telemetry pkg/embed/fs
