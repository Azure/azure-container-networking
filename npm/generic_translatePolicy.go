...
func (t *translator) defaultDropACL(npmNetpol *policies.NPMNetworkPolicy, direction policies.Direction) {
	dropACL := policies.NewACLPolicy(npmNetpol.Name, npmNetpol.NameSpace, policies.Dropped, direction)
...
func (t *translator) translateIngress(npmNetpol *policies.NPMNetworkPolicy, targetSelector metav1.LabelSelector, rules []networkingv1.NetworkPolicyIngressRule) error {
	...
	for i, rule := range rules {
		...
		if !portRuleExists && !fromRuleExists && !allowExternal {
			acl := policies.NewACLPolicy(npmNetpol.Name, npmNetpol.NameSpace, policies.Allowed, policies.Ingress)
		...
		if portRuleExists && !fromRuleExists && !allowExternal {
			for _, portRule := range rule.Ports {
				...
				portACL := policies.NewACLPolicy(npmNetpol.Name, npmNetpol.NameSpace, policies.Allowed, policies.Ingress)
		...
		for j, fromRule := range rule.From {
			...
			if fromRule.IPBlock != nil {
				if len(fromRule.IPBlock.CIDR) > 0 {
					...
					if !portRuleExists {
						fromRuleACL := policies.NewACLPolicy(npmNetpol.Name, npmNetpol.NameSpace, policies.Allowed, policies.Ingress)
						fromRuleACL.SrcList = append(fromRuleACL.SrcList, ipBlockSetInfo)
						npmNetpol.ACLs = append(npmNetpol.ACLs, fromRuleACL)
					} else {
						for _, portRule := range rule.Ports {
							ipBlockAndPortRuleACL := policies.NewACLPolicy(npmNetpol.Name, npmNetpol.NameSpace, policies.Allowed, policies.Ingress)
			...
			if fromRule.PodSelector == nil && fromRule.NamespaceSelector != nil {
				for _, nsSelector := range FlattenNameSpaceSelector(fromRule.NamespaceSelector) {
					...
					if !portRuleExists {
						nsACL := policies.NewACLPolicy(npmNetpol.Name, npmNetpol.NameSpace, policies.Allowed, policies.Ingress)
					...
					for _, portRule := range rule.Ports {
						...
						nsAndPortACL := policies.NewACLPolicy(npmNetpol.Name, npmNetpol.NameSpace, policies.Allowed, policies.Ingress)
			...
			if fromRule.PodSelector != nil && fromRule.NamespaceSelector == nil {
				...
				for _, portRule := range rule.Ports {
					...
					podAndPortACL := policies.NewACLPolicy(npmNetpol.Name, npmNetpol.NameSpace, policies.Allowed, policies.Ingress)
			...
			for _, nsSelector := range FlattenNameSpaceSelector(fromRule.NamespaceSelector) {
				...
				for _, portRule := range rule.Ports {
					...
					aclWithAllFields := policies.NewACLPolicy(npmNetpol.Name, npmNetpol.NameSpace, policies.Allowed, policies.Ingress)
		...
		if allowExternal {
			allowExternalACL := policies.NewACLPolicy(npmNetpol.Name, npmNetpol.NameSpace, policies.Allowed, policies.Ingress)
