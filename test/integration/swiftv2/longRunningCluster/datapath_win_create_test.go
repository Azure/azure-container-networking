//go:build create_windows_test
// +build create_windows_test
...
func TestDatapathCreateWindows(t *testing.T) {
	gomega.RegisterFailHandler(ginkgo.Fail)
	ginkgo.RunSpecs(t, "Datapath Create Windows Suite")
}

var _ = ginkgo.Describe("Datapath Create Windows Tests", func() {

	ginkgo.It("creates PodNetwork, PodNetworkInstance, and Windows Pods", func() {
		rg := os.Getenv("RG")
		...
		// Define all Windows test scenarios. Names mirror the Linux create test, with
		// "-win" appended to PodNameSuffix so Windows pods can coexist with Linux pods
		// in the same namespace if both were ever provisioned together.
		...
		ginkgo.By(fmt.Sprintf("Creating all windows test scenarios (%d scenarios)", len(scenarios)))
		err := CreateAllScenarios(testScenarios)
		gomega.Expect(err).To(gomega.BeNil(), "Failed to create windows test scenarios")

		ginkgo.By("Successfully created all windows test scenarios")
	})
})
