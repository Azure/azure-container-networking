...
The `npwin` and `npwinl` pools are gated by the `enableManagedWindows` flag on each scenario in `pipeline.yaml` and are **off by default**. When enabled, `create_aks.sh` first ensures the cluster has Windows admin credentials (via `az aks update --windows-admin-*` if not already set) and then adds both Windows swiftv2 pools (high-NIC `npwin` and low-NIC `npwinl`) with the same multi-tenancy tags and headers as `nplinux` / `nodepool1`. Datapath tests targeting `workload-type=swiftv2-windows` are wired in via `template/datapath-windows-tests-stage.yaml` and run create / connectivity / delete jobs using the `*_windows_test` build tags (no PrivateEndpoint or Scale jobs). The datapath stage is gated on **both** `enableManagedWindows` (so the pools exist) **and** a separate `enableDatapathWindows` flag, mirroring the Linux split between infra flags (`enableLinuxBYON` / `enableAccelnet`) and the datapath flag (`enableDatapathLinux`). This lets a user provision the Windows pools without running the datapath tests, or vice versa.
