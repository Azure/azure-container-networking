  ...
  while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
    if az storage blob upload \
      ...
      if [ $RETRY_COUNT -lt $MAX_RETRIES ]; then
        echo "[WARN] Blob upload failed (attempt $RETRY_COUNT/$MAX_RETRIES). Waiting ${SLEEP_TIME}s for RBAC propagation..."
        sleep $SLEEP_TIME
...
# Note: RBAC role is kept active for SAS token generation during tests
# It will be removed during cleanup phase
