...
5. Weekly digest (`weekly-trends-pipeline.yml`): set `parameters.analysisPipelineIds` to the ADO build-definition id(s) whose `failureAnalysis_*` artifacts to aggregate, enable "Allow scripts to access the OAuth token" (for `SYSTEM_ACCESSTOKEN`), provide the `AZURE_OPENAI_*` variables to the synthesis step, and ensure `Go`, `curl`, `jq`, and `unzip` are on the agent.
