...
| `scripts/render-verdict.sh` | Shared `render_verdict <incident.json>` jq helper — renders the full FAA verdict (final verdict, top anomaly, failing unit, causal chain, symptom-vs-cause, falsification, node assessment, evidence, gaps, recommended action / proposed fix). Each section is type-guarded, so a malformed field skips only its own section. |
